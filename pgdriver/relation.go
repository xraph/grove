...
func (q *SelectQuery) loadRelations(ctx context.Context, target any) error {
	...
	for _, relName := range q.relations {
		...
		if !ok {
			return fmt.Errorf("pgdriver: unknown relation %q on table %q", relName, q.table.Name)
		}

		switch rel.Type {
		case schema.HasOne:
			if err := q.loadHasOne(ctx, target, rel); err != nil {
				return fmt.Errorf("pgdriver: load has-one %q: %w", relName, err)
			}
		case schema.HasMany:
			if err := q.loadHasMany(ctx, target, rel); err != nil {
				return fmt.Errorf("pgdriver: load has-many %q: %w", relName, err)
			}
		case schema.BelongsTo:
			if err := q.loadBelongsTo(ctx, target, rel); err != nil {
				return fmt.Errorf("pgdriver: load belongs-to %q: %w", relName, err)
			}
		default:
			return fmt.Errorf("pgdriver: relation type %s not yet supported for eager loading", rel.Type)
		}
	}
...
// loadHasOne loads a has-one relation: SELECT * FROM related WHERE join_col = base_val.
...
// loadHasMany loads a has-many relation: SELECT * FROM related WHERE join_col = base_val.
...
func (q *SelectQuery) loadHasManyForSlice(ctx context.Context, sliceVal reflect.Value, rel *schema.Relation) error {
	...
	// Build IN query: SELECT * FROM related WHERE join_col IN ($1, $2, ...)
...
// loadBelongsTo loads a belongs-to relation: SELECT * FROM related WHERE base_col = join_val.
...
// buildRelationQuery builds a SELECT * FROM table WHERE col = $1 query.
func buildRelationQuery(dialect *PgDialect, table *schema.Table, col string, val any) (string, []any) {
	buf := pool.GetBuffer()
...
// buildRelationInQuery builds a SELECT * FROM table WHERE col IN ($1, $2, ...) query.
func buildRelationInQuery(dialect *PgDialect, table *schema.Table, col string, vals []any) (string, []any) {
	buf := pool.GetBuffer()
	defer pool.PutBuffer(buf)

...
func (q *SelectQuery) loadRelations(ctx context.Context, target any) error {
	...
	for _, relName := range q.relations {
		...
		if !ok {
			return fmt.Errorf("sqlitedriver: unknown relation %q on table %q", relName, q.table.Name)
		}

		switch rel.Type {
		case schema.HasOne:
			if err := q.loadHasOne(ctx, target, rel); err != nil {
				return fmt.Errorf("sqlitedriver: load has-one %q: %w", relName, err)
			}
		case schema.HasMany:
			if err := q.loadHasMany(ctx, target, rel); err != nil {
				return fmt.Errorf("sqlitedriver: load has-many %q: %w", relName, err)
			}
		case schema.BelongsTo:
			if err := q.loadBelongsTo(ctx, target, rel); err != nil {
				return fmt.Errorf("sqlitedriver: load belongs-to %q: %w", relName, err)
			}
		default:
			return fmt.Errorf("sqlitedriver: relation type %s not yet supported for eager loading", rel.Type)
		}
	}
...
func buildRelationQuery(dialect *SqliteDialect, table *schema.Table, col string, val any) (string, []any) {
	buf := pool.GetBuffer()
...
func buildRelationInQuery(dialect *SqliteDialect, table *schema.Table, col string, vals []any) (string, []any) {
	buf := pool.GetBuffer()
	defer pool.PutBuffer(buf)

...
func (q *InsertQuery) Exec(ctx context.Context) (driver.Result, error) {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return nil, fmt.Errorf("sqlitedriver: insert denied by hook")
		}
	}
...
func (q *InsertQuery) Scan(ctx context.Context, dest ...any) error {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return fmt.Errorf("sqlitedriver: insert denied by hook")
		}
	}
...
func (q *InsertQuery) execPrepared(ctx context.Context, qc *hook.QueryContext) (driver.Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlitedriver: bulk insert begin tx: %w", err)
	}

	stx := &SqliteTx{db: q.db, tx: tx}
	txdb := stx.txDB()
	...
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("sqlitedriver: bulk insert commit: %w", err)
	}
...
func (q *InsertQuery) execPreparedWith(ctx context.Context, db *SqliteDB, fields []*schema.Field, query string) (driver.Result, error) {
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			return nil, fmt.Errorf("sqlitedriver: bulk insert row %d: %w", i, err)
		}
...
func (r *bulkResult) LastInsertId() (int64, error) {
	return 0, fmt.Errorf("sqlitedriver: LastInsertId not available for bulk insert")
}
...
func (q *InsertQuery) extractModelValues(fields []*schema.Field) ([][]any, error) {
	...
	for val.Kind() == reflect.Ptr {
		if val.IsNil() {
			return nil, fmt.Errorf("sqlitedriver: nil model pointer")
		}
		val = val.Elem()
	}
	...
	if val.Kind() == reflect.Slice {
		...
		n := val.Len()
		if n == 0 {
			return nil, fmt.Errorf("sqlitedriver: empty slice for bulk insert")
		}
		rows := make([][]any, n)
		for i := 0; i < n; i++ {
			elem := val.Index(i)
	...
	return nil, fmt.Errorf("sqlitedriver: unsupported model type %v", val.Kind())
}

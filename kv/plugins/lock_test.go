...
func TestLock_Acquire_Success(t *testing.T) {
	...
	lock := plugins.NewLock(store, "test-resource", 5*time.Second)
...
func TestLock_Acquire_AlreadyHeld(t *testing.T) {
	...
	lock2 := plugins.NewLock(store, "test-resource", 5*time.Second)
...
func TestLock_AcquireWithRetry_Success(t *testing.T) {
	...
	holder := plugins.NewLock(store, "retry-resource", 5*time.Second)
	...
	contender := plugins.NewLock(store, "retry-resource", 5*time.Second)
...
func TestLock_AcquireWithRetry_MaxRetries(t *testing.T) {
	...
	holder := plugins.NewLock(store, "held-forever", 1*time.Minute)
	err := holder.Acquire(ctx)
	require.NoError(t, err)

	contender := plugins.NewLock(store, "held-forever", 5*time.Second)
...
func TestLock_AcquireWithRetry_ContextCancel(t *testing.T) {
	...
	holder := plugins.NewLock(store, "cancel-resource", 1*time.Minute)
	...
	contender := plugins.NewLock(store, "cancel-resource", 5*time.Second)
...
func TestLock_Extend_Success(t *testing.T) {
	store := kvtest.SetupStore(t)
	lock := plugins.NewLock(store, "extend-resource", 5*time.Second)
...
func TestLock_TTL_Expiry(t *testing.T) {
	...
	lock1 := plugins.NewLock(store, "expiry-resource", 50*time.Millisecond)
	...
	lock2 := plugins.NewLock(store, "expiry-resource", 5*time.Second)
	err = lock2.Acquire(ctx)
	require.NoError(t, err)
}

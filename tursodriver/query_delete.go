...
// DeleteQuery builds Turso/libSQL DELETE statements.
...
func (db *TursoDB) NewDelete(model any) *DeleteQuery {
	q := &DeleteQuery{
		db:    db,
		model: model,
	}
...
func (q *DeleteQuery) Exec(ctx context.Context) (driver.Result, error) {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return nil, fmt.Errorf("tursodriver: delete denied by hook")
		}
	}
...
func (q *DeleteQuery) Scan(ctx context.Context, dest ...any) error {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return fmt.Errorf("tursodriver: delete denied by hook")
		}
	}

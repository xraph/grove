...
// CreateTableQuery builds Turso/libSQL CREATE TABLE statements.
...
func (db *TursoDB) NewCreateTable(model any) *CreateTableQuery {
	q := &CreateTableQuery{
		db:    db,
		model: model,
	}
...
// resolveColumnType determines the SQLite/Turso column type for a field.
...
// DropTableQuery builds Turso/libSQL DROP TABLE statements.
...
func (db *TursoDB) NewDropTable(model any) *DropTableQuery {
	q := &DropTableQuery{
		db:    db,
		model: model,
	}

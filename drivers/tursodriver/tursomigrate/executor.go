// Package tursomigrate provides a Turso/libSQL-specific migration executor
// for the Grove migration system.
package tursomigrate
...
func init() {
	migrate.RegisterExecutor("turso", func(drv any) migrate.Executor {
...
// Executor implements migrate.Executor for Turso/libSQL.
...
// New creates a new Turso/libSQL migration executor.
...
// approach. Turso/SQLite is single-writer by nature, so this provides a simple
...
func (e *Executor) AcquireLock(ctx context.Context, lockedBy string) error {
	...
	if err != nil {
		return fmt.Errorf("tursomigrate: ensure lock row: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("tursomigrate: acquire lock: %w", err)
	}

	n, err := res.RowsAffected()
	if err != nil {
		return fmt.Errorf("tursomigrate: check lock acquisition: %w", err)
	}
	if n == 0 {
		return fmt.Errorf("tursomigrate: migration lock is held by another process")
	}
...
func (e *Executor) ListApplied(ctx context.Context) ([]*migrate.AppliedMigration, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.Version, &a.Name, &a.Group, &a.MigratedAt); err != nil {
			return nil, fmt.Errorf("tursomigrate: scan applied: %w", err)
		}

...
// SelectQuery builds Turso/libSQL SELECT statements.
...
func (db *TursoDB) NewSelect(model ...any) *SelectQuery {
	q := &SelectQuery{}
	q.db = db
...
// It generates conditions like "table"."pk_col" = ? using placeholders.
func (q *SelectQuery) WherePK() *SelectQuery {
	if q.table == nil {
		q.err = fmt.Errorf("tursodriver: WherePK requires a model with table metadata")
		return q
	}
...
func (q *SelectQuery) Scan(ctx context.Context, dest ...any) error {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return fmt.Errorf("tursodriver: query denied by hook")
		}
	...
	if target == nil {
		return fmt.Errorf("tursodriver: Scan requires a destination; pass a model to NewSelect or provide dest")
	}
...
func (q *SelectQuery) Count(ctx context.Context) (int64, error) {
	...
	if err := row.Scan(&count); err != nil {
		return 0, fmt.Errorf("tursodriver: count: %w", err)
	}
	return count, nil
}

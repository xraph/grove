...
func TestSelectQuery_WherePK(t *testing.T) {
	...
	expected := `SELECT "u"."id", "u"."name", "u"."email", "u"."role", "u"."created_at", "u"."updated_at", "u"."deleted_at" FROM "users" AS "u" WHERE "u"."id" = $1 AND "u"."deleted_at" IS NULL`
	assert.Equal(t, expected, sql)
...
func TestSelectQuery_DistinctOn(t *testing.T) {
	...
	assert.Contains(t, sql, `DISTINCT ON ("u"."email")`)
	assert.Empty(t, args)
}
...
func TestSelectQuery_ForUpdate(t *testing.T) {
	...
	assert.NotContains(t, sql, "FOR SHARE")
	assert.Empty(t, args)
}

func TestSelectQuery_ForUpdateOfTable(t *testing.T) {
	db := newTestDB()
	...
	assert.Contains(t, sql, `FOR UPDATE OF "users"`)
	assert.Empty(t, args)
}
...
func TestSelectQuery_WhereWithArgs(t *testing.T) {
	...
	// The select query WHERE stores args but does not replace ? placeholders itself;
	// the user is expected to use $N placeholders or literal values in SELECT wheres.
	// The args are accumulated in q.args during appendWheres.
...
func TestInsertQuery_BasicFromModel(t *testing.T) {
	...
	assert.Contains(t, sql, "VALUES ($1, $2, $3, $4, $5, $6)")
...
func TestInsertQuery_OnConflictUpsert(t *testing.T) {
	...
	// args should contain the INSERT values only (SET uses EXCLUDED, no extra args).
...
func TestInsertQuery_BulkInsert(t *testing.T) {
	...
	// Check placeholder structure: ($1, $2), ($3, $4), ($5, $6)
...
func TestUpdateQuery_FromModelAllFields(t *testing.T) {
	...
	assert.Contains(t, sql, `"role" = $3`)
	assert.Contains(t, sql, `"created_at" = $4`)
	assert.Contains(t, sql, `"updated_at" = $5`)
	assert.Contains(t, sql, `"deleted_at" = $6`)
...
func TestUpdateQuery_ColumnSpecific(t *testing.T) {
	...
	assert.Contains(t, sql, `UPDATE "users" SET "name" = $1, "role" = $2`)
...
func TestUpdateQuery_PostBasic(t *testing.T) {
	...
	assert.Contains(t, sql, `UPDATE "posts" SET "title" = $1`)
	assert.Contains(t, sql, `WHERE "id" = $2`)
	require.Len(t, args, 2)
...
func TestDeleteQuery_HardDeleteWithWhere(t *testing.T) {
	...
	expected := `DELETE FROM "posts" WHERE "user_id" = $1`
	assert.Equal(t, expected, sql)
	require.Len(t, args, 1)
	assert.Equal(t, int64(42), args[0])
}
...
func TestDeleteQuery_HardDeleteWithWherePK(t *testing.T) {
	...
	expected := `DELETE FROM "posts" WHERE "id" = $1`
	assert.Equal(t, expected, sql)
	require.Len(t, args, 1)
...
func TestDeleteQuery_SoftDeleteWithWhereClause(t *testing.T) {
	...
	assert.Contains(t, sql, `UPDATE "users" SET "deleted_at" = NOW()`)
	assert.Contains(t, sql, `WHERE "role" = $1`)
	require.Len(t, args, 1)
...
func TestDeleteQuery_MultipleWhereConditions(t *testing.T) {
	...
	expected := `DELETE FROM "posts" WHERE "user_id" = $1 AND "title" = $2`
	assert.Equal(t, expected, sql)
	require.Len(t, args, 2)
...
func TestSelectQuery_Variants(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantSQL:  `SELECT "id", "title", "user_id" FROM "posts" WHERE "id" = $1`,
			wantArgs: []any{int64(7)},
		},
		{
			name: "User DistinctOn + OrderBy",
			build: func() (string, []any, error) {
				return db.NewSelect((*TestUser)(nil)).
					DistinctOn(`"u"."email"`).
					OrderExpr(`"u"."email"`).
					OrderExpr(`"u"."created_at" DESC`).
					Build()
			},
			wantSQL:  `SELECT DISTINCT ON ("u"."email") "u"."id", "u"."name", "u"."email", "u"."role", "u"."created_at", "u"."updated_at", "u"."deleted_at" FROM "users" AS "u" WHERE "u"."deleted_at" IS NULL ORDER BY "u"."email", "u"."created_at" DESC`,
			wantArgs: nil,
		},
	}
...
func TestUpdateQuery_Variants(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  `UPDATE "posts" SET "title" = $1 WHERE "id" = $2`,
		...
		},
		{
			...
			wantSQL:  `UPDATE "posts" SET "title" = $1, "user_id" = $2 WHERE "id" = $3`,
		...
		},
		{
			...
			wantSQL:  `UPDATE "posts" SET "title" = $1 WHERE "id" = $2 RETURNING "id", "title"`,
...
// =========================================================================
// WhereArray, TableExpr, Lateral tests
// =========================================================================

func TestSelectQuery_WhereArray(t *testing.T) {
	db := newTestDB()

	sql, args, err := db.NewSelect((*TestUser)(nil)).
		WhereArray("role", "= ANY", []string{"admin", "mod"}).
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, `"role" = ANY ($1)`)
	require.Len(t, args, 1)
	assert.Equal(t, []string{"admin", "mod"}, args[0])
}

func TestSelectQuery_WhereArray_WithExistingWhere(t *testing.T) {
	db := newTestDB()

	sql, args, err := db.NewSelect((*TestUser)(nil)).
		Where(`"u"."name" = $1`, "Alice").
		WhereArray("role", "= ANY", []string{"admin", "editor"}).
		Build()
	require.NoError(t, err)

	// The first WHERE clause uses $1, so WhereArray should use $2.
	assert.Contains(t, sql, `"u"."name" = $1`)
	assert.Contains(t, sql, `"role" = ANY ($2)`)
	require.Len(t, args, 2)
	assert.Equal(t, "Alice", args[0])
	assert.Equal(t, []string{"admin", "editor"}, args[1])
}

func TestSelectQuery_WhereArray_AllOperator(t *testing.T) {
	db := newTestDB()

	sql, args, err := db.NewSelect((*TestPost)(nil)).
		WhereArray("user_id", "<> ALL", []int64{1, 2, 3}).
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, `"user_id" <> ALL ($1)`)
	require.Len(t, args, 1)
	assert.Equal(t, []int64{1, 2, 3}, args[0])
}
...
func TestSelectQuery_TableExpr(t *testing.T) {
	...
	sql, args, err := db.NewSelect().
		ColumnExpr("s.n").
		TableExpr("generate_series(1, 10) AS s(n)").
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, "FROM generate_series(1, 10) AS s(n)")
	assert.Contains(t, sql, "SELECT s.n")
	assert.Empty(t, args)
}

func TestSelectQuery_TableExpr_WithArgs(t *testing.T) {
	db := newTestDB()

	sql, args, err := db.NewSelect().
		ColumnExpr("*").
		TableExpr("generate_series($1, $2) AS s(n)", 1, 100).
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, "FROM generate_series($1, $2) AS s(n)")
	require.Len(t, args, 2)
	assert.Equal(t, 1, args[0])
	assert.Equal(t, 100, args[1])
}

func TestSelectQuery_TableExpr_OverridesModelTable(t *testing.T) {
	db := newTestDB()

	// Even when a model is provided, TableExpr should override the FROM clause.
	sql, _, err := db.NewSelect((*TestPost)(nil)).
		TableExpr("my_custom_view AS v").
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, "FROM my_custom_view AS v")
	assert.NotContains(t, sql, `FROM "posts"`)
}

func TestSelectQuery_Lateral(t *testing.T) {
	db := newTestDB()

	sql, args, err := db.NewSelect((*TestUser)(nil)).
		Lateral("SELECT * FROM posts WHERE posts.user_id = u.id LIMIT 3", "lat").
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, "JOIN LATERAL")
	assert.Contains(t, sql, "(SELECT * FROM posts WHERE posts.user_id = u.id LIMIT 3)")
	assert.Contains(t, sql, `AS "lat"`)
	assert.Contains(t, sql, "ON true")
	assert.Empty(t, args)
}

func TestSelectQuery_Lateral_WithArgs(t *testing.T) {
	db := newTestDB()

	sql, args, err := db.NewSelect((*TestUser)(nil)).
		Lateral("SELECT * FROM posts WHERE posts.user_id = u.id LIMIT $1", "recent_posts", 5).
		Build()
	require.NoError(t, err)

	assert.Contains(t, sql, "JOIN LATERAL")
	assert.Contains(t, sql, `AS "recent_posts"`)
	assert.Contains(t, sql, "ON true")
	require.Len(t, args, 1)
	assert.Equal(t, 5, args[0])
}

func TestSelectQuery_Lateral_BuildCount(t *testing.T) {
	db := newTestDB()

	sql, _, err := db.NewSelect((*TestUser)(nil)).
		Lateral("SELECT * FROM posts WHERE posts.user_id = u.id", "lat").
		BuildCount()
	require.NoError(t, err)
...
func TestDeleteQuery_Variants(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  `DELETE FROM "posts" WHERE "id" = $1`,
		...
		},
		{
			...
			wantSQL:  `DELETE FROM "posts" WHERE "id" = $1 RETURNING "id"`,
...
// =========================================================================
// STREAMING & CDC CAPABILITY TESTS
// =========================================================================

func TestPgDB_SupportsStreaming(t *testing.T) {
	db := newTestDB()
	assert.True(t, db.SupportsStreaming(), "PgDB should support streaming")
}

func TestPgDB_SupportsCDC(t *testing.T) {
	db := newTestDB()
	assert.True(t, db.SupportsCDC(), "PgDB should support CDC")
}
...
func TestPgDB_GroveSelect_MatchesNewSelect(t *testing.T) {
	db := newTestDB()
...
func TestPgDB_GroveInsert_MatchesNewInsert(t *testing.T) {
	db := newTestDB()
...
func TestPgDB_GroveUpdate_MatchesNewUpdate(t *testing.T) {
	db := newTestDB()
...
func TestPgDB_GroveDelete_MatchesNewDelete(t *testing.T) {
	db := newTestDB()
	post := &TestPost{ID: 1}

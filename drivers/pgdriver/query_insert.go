...
func (q *InsertQuery) execPrepared(ctx context.Context, qc *hook.QueryContext) (driver.Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pgdriver: bulk insert begin tx: %w", err)
	}

	ptx := &PgTx{db: q.db, tx: tx}
	txdb := ptx.txDB()
...
func (q *InsertQuery) execPreparedWith(ctx context.Context, db *PgDB, fields []*schema.Field, query string) (driver.Result, error) {
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			return nil, fmt.Errorf("pgdriver: bulk insert row %d: %w", i, err)
		}
		affected, _ := res.RowsAffected()
		totalAffected += affected
	}

	return &pgBulkResult{rowsAffected: totalAffected}, nil
}
...
// pgBulkResult implements driver.Result for prepared-statement bulk inserts.
type pgBulkResult struct {
	rowsAffected int64
}

func (r *pgBulkResult) RowsAffected() (int64, error) { return r.rowsAffected, nil }
func (r *pgBulkResult) LastInsertId() (int64, error) {

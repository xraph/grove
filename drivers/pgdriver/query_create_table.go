...
// resolveColumnType determines the PostgreSQL column type for a field.
...
func (q *CreateTableQuery) resolveColumnType(f *schema.Field) string {
	...
	// AutoIncrement fields use SERIAL / BIGSERIAL.
...
// produces `DEFAULT active`, which PostgreSQL parses as a column reference
// ("cannot use column reference in DEFAULT expression"). Anything that isn't a
// number, boolean/NULL/keyword, cast, or function-call expression is therefore
// treated as a string literal and quoted. Values that already carry quotes are

...
func resolveTable(reg *schema.Registry, model any) (*schema.Table, error) {
	if model == nil {
		return nil, fmt.Errorf("clickhousedriver: nil model")
	}
	...
	if typ.Kind() != reflect.Struct {
		return nil, fmt.Errorf("clickhousedriver: model must be a struct or pointer/slice of struct, got %v", typ.Kind())
	}

...
// SelectQuery builds ClickHouse SELECT statements.
type SelectQuery struct {
	...
	useFinal      bool     // FINAL modifier after table name
	sampleRate    float64  // SAMPLE clause value (0 means no SAMPLE)
	prewheres     []whereClause
}
...
func (db *ClickHouseDB) NewSelect(model ...any) *SelectQuery {
	q := &SelectQuery{}
	q.db = db
...
// it from the model's table name.
...
func (q *SelectQuery) WherePK() *SelectQuery {
	if q.table == nil {
		q.err = fmt.Errorf("clickhousedriver: WherePK requires a model with table metadata")
		return q
	}
...
// Final adds the FINAL modifier after the table name.
// This causes ClickHouse to merge rows from ReplacingMergeTree or
// CollapsingMergeTree before returning results.
func (q *SelectQuery) Final() *SelectQuery {
	q.useFinal = true
	return q
}

// Sample adds a SAMPLE clause. The rate should be between 0 and 1
// (e.g., 0.1 for 10% of data) or an integer for exact row count.
func (q *SelectQuery) Sample(n float64) *SelectQuery {
	q.sampleRate = n
	return q
}

// Prewhere adds a PREWHERE clause. ClickHouse evaluates PREWHERE before
// reading columns not needed for the filter, which can significantly
// improve query performance on wide tables.
func (q *SelectQuery) Prewhere(query string, args ...any) *SelectQuery {
	q.prewheres = append(q.prewheres, whereClause{query: query, args: args, sep: "AND"})
	return q
}
...
func (q *SelectQuery) applySoftDeleteFilter() {
	...
	// Check if we already added this filter (idempotency).
...
func (q *SelectQuery) Build() (string, []any, error) {
	...
	// FINAL modifier (ClickHouse-specific).
	if q.useFinal {
	...
	if q.sampleRate > 0 {
		...
		buf.WriteString(strconv.FormatFloat(q.sampleRate, 'f', -1, 64))
	}
	...
	if len(q.prewheres) > 0 {
		buf.WriteString(" PREWHERE ")
		for i, pw := range q.prewheres {
			if i > 0 {
				_ = buf.WriteByte(' ')
				buf.WriteString(pw.sep)
				_ = buf.WriteByte(' ')
			}
			buf.WriteString(pw.query)
			q.args = append(q.args, pw.args...)
		}
	}
...
func (q *SelectQuery) Scan(ctx context.Context, dest ...any) error {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return fmt.Errorf("clickhousedriver: query denied by hook")
		}
	...
	if target == nil {
		return fmt.Errorf("clickhousedriver: Scan requires a destination; pass a model to NewSelect or provide dest")
	}
...
func (q *SelectQuery) Count(ctx context.Context) (int64, error) {
	...
	if err := row.Scan(&count); err != nil {
		return 0, fmt.Errorf("clickhousedriver: count: %w", err)
	}
	return count, nil
}

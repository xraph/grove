...
func (q *SelectQuery) WherePK() *SelectQuery {
	if q.table == nil {
		q.err = fmt.Errorf("sqlitedriver: WherePK requires a model with table metadata")
		return q
	}
...
func (q *SelectQuery) Scan(ctx context.Context, dest ...any) error {
	...
	if q.db.hooks != nil {
		...
		if result != nil && result.Decision == hook.Deny {
			...
			return fmt.Errorf("sqlitedriver: query denied by hook")
		}
...
func (q *SelectQuery) Count(ctx context.Context) (int64, error) {
	...
	if err := row.Scan(&count); err != nil {
		return 0, fmt.Errorf("sqlitedriver: count: %w", err)
	}
	return count, nil
}

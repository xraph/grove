...
sessions := extension.NewSessionStore(store)
limiter  := extension.NewRateLimiter(store, "api", 100, time.Minute)
lock     := extension.NewLock(store, "deploy", 10*time.Second)
counter  := extension.NewAtomicCounter(store, "visitors")
board    := extension.NewLeaderboard(store, "scores")
queue    := extension.NewQueue(store, "tasks")
```

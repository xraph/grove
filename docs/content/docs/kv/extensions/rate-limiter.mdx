...
```go
import "github.com/xraph/grove/kv/extension"
```

## Creating a Rate Limiter

```go
limiter := extension.NewRateLimiter(store, "api", 100, time.Minute)
```
...
func RateLimitMiddleware(limiter *extension.RateLimiter, next http.HandlerFunc) http.HandlerFunc {
...
authLimiter := extension.NewRateLimiter(store, "auth", 5, time.Minute)

// Generous limit on read endpoints.
readLimiter := extension.NewRateLimiter(store, "read", 1000, time.Minute)

// Or use a single limiter with composite keys.
limiter := extension.NewRateLimiter(store, "rl", 100, time.Minute)
